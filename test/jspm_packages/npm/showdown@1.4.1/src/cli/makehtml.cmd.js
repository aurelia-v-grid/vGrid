...
(function(process) {
  var yargs = require('yargs'),
      fs = require('fs'),
      errorExit = require('./errorexit'),
      showdown = require('../../dist/showdown');
  yargs.reset().usage('Usage: showdown makehtml [options]').example('showdown makehtml -i', 'Reads from stdin and outputs to stdout').example('showdown makehtml -i foo.md -o bar.html', 'Reads \'foo.md\' and writes to \'bar.html\'').option('i', {
    alias: 'input',
    ...
    type: 'array'
  }).config('c').alias('c', 'config').help('h').alias('h', 'help');
