...
exports['language:de-ch'] = {
  ...
  percentages: function(test) {
    test.expect(4);
    var tests = [[1, '0%', '100%'], [.974878234, '0.000%', '97,488%'], [-.43, '0%', '-43%'], [.43, '(0.000%)', '43,000%']];
    for (var i = 0; i < tests.length; i++) {
      test.strictEqual(numeral(tests[i][0]).format(tests[i][1]), tests[i][2], tests[i][1]);
    }
    test.done();
  ...
  unformat: function(test) {
    test.expect(9);
    var tests = [['10 000,123', 10000.123], ['(0,12345)', -.12345], ['(CHF1,23m)', -1230000], ['10k', 10000], ['-10k', -10000], ['23.', 23], ['CHF10 000,00', 10000], ['-76%', -.76], ['2:23:57', 8637]];
    for (var i = 0; i < tests.length; i++) {
